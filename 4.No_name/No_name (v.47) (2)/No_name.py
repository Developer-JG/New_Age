...
# 플레이어 클래스
class Player:
    def __init__(player_1, name, point, equipment, inven, health, max_health, money, damage, critical, defense, liv, exp, po_count, ag_count,
                 ad_count, player_x, player_y):
        player_1.name = name
        player_1.point = point
        player_1.equipment = {"atk": ["없음"], "def_1": ["없음"], "def_2": ["없음"], "def_3": ["없음"], "def_4": ["없음"]}
        player_1.inven = {"atk": [], "def": [], "potion": [], "month": [], "item": []}
        player_1.health = health
        player_1.max_health = max_health
        player_1.money = money
        player_1.damage = damage
        player_1.critical = critical
        player_1.defense = defense
        player_1.liv = liv
        player_1.exp = exp
        player_1.po_count = po_count
        player_1.ag_count = ag_count
        player_1.ad_count = ad_count
        player_1.player_x = player_x
        player_1.player_y = player_y
...
# 흰색 (노말) 무기 아이템 클래스
class W_atk_item:
    def __init__(player_1, name, damage, month, cost, liv, need_power_stats, need_agility_stats, need_adventure_stats, critical,
                 physical_absorption, use):
        player_1.name = name
        player_1.damage = damage
        player_1.month = month
        player_1.cost = cost
        player_1.liv = liv
        player_1.need_power_stats = need_power_stats
        player_1.need_agility_stats = need_agility_stats
        player_1.need_adventure_stats = need_adventure_stats
        player_1.critical = critical
        player_1.physical_absorption = physical_absorption
        player_1.use = use
...
# 흰색 (노말) 방어구 아이템 클래스
class W_def_item:
    def __init__(player_1, name, defense, month, cost, part, liv, need_power_stats, need_agility_stats, need_adventure_stats,
        ...
        player_1.need_agility_stats = need_agility_stats
        player_1.need_adventure_stats = need_adventure_stats
        player_1.plus_power_stats = plus_power_stats
        player_1.plus_agility_stats = plus_agility_stats
        player_1.physical_absorption = physical_absorption
        player_1.use = use
...
# 보라색 (에픽) 무기 아이템 클래스
class P_atk_item:
    def __init__(player_1, name, damage, liv, critical, physical_absorption, use):
        player_1.name = name
        player_1.damage = damage
        player_1.liv = liv
        player_1.critical = critical
...
# 보라색 (에픽) 방어구 아이템 클래스
class P_def_item:
    def __init__(player_1, name, defense, liv, part, need_power_stats, need_agility_stats, need_adventure_stats, use):
        player_1.name = name
        player_1.defense = defense
        player_1.liv = liv
        player_1.part = part
...
class R_r_atk_item:
    def __init__(player_1, name, damage, need_power_stats, need_agility_stats, need_adventure_stats, critical,
                 physical_absorption, use):
        player_1.name = name
        player_1.damage = damage
        player_1.need_power_stats = need_power_stats
...
class R_r_def_item:
    def __init__(player_1, name, defense, part, need_power_stats, need_agility_stats, need_adventure_stats,
        ...
        player_1.plus_power_stats = plus_power_stats
        player_1.plus_agility_stats = plus_agility_stats
        player_1.plus_adventure_stats = plus_adventure_stats
        player_1.use = use
...
class R_s_atk_item:
    def __init__(player_1, name, damage, liv, need_power_stats, need_agility_stats, need_adventure_stats, critical,
                 physical_absorption, use):
        player_1.name = name
        player_1.damage = damage
        player_1.liv = liv
...
class R_s_def_item:
    def __init__(player_1, name, defense, part, liv, plus_power_stats, plus_agility_stats, plus_adventure_stats, use):
        player_1.name = name
        player_1.defense = defense
        player_1.liv = liv
        player_1.part = part
...
class R_b_atk_item:
    def __init__(player_1, name, damage, liv, critical, use):
        player_1.name = name
        player_1.damage = damage
        player_1.liv = liv
        player_1.critical = critical
...
# 선과 악 아이템 클래스
class g_e_atk_item:
    def __init__(player_1, name, damage, critical, physical_absorption, use):
        player_1.name = name
        player_1.damage = damage
        player_1.critical = critical
        player_1.physical_absorption = physical_absorption
...
class fortification_item:
    def __init__(player_1, name, cost, count, use_effect):
        player_1.name = name
        player_1.cost = cost
        player_1.count = count
        player_1.use_effect = use_effect

# 강화서 (개척자의 증표로 구매)
fortification_item_101 = fortification_item("하급 무기강화", 1, 0, "+ 3~18 데미지 (레벨 제한 3)")
fortification_item_201 = fortification_item("중급 무기강화", 1, 0, "+ 3~27 데미지 (레벨 제한 4)")
fortification_item_202 = fortification_item("고급 무기강화", 3, 0, "+ 18~27 데미지 (레벨 제한 5)")
fortification_item_203 = fortification_item("힘 강화", 1, 0, "+ 3~9 데미지 (힘 스텟 제한 20")
fortification_item_204 = fortification_item("민첩 강화", 1, 0, "크리티컬 + 3~5% (민첩 스텟 제한 20)")
fortification_item_205 = fortification_item("모험 강화", 1, 0, "체력흡수 + 16~39 (모험 스텟 제한 20)")
fortification_item_206 = fortification_item("방어구 강화", 3, 0, "+ 3~18 방어력 (레벨 제한 10)")
fortification_item_301 = fortification_item("저주받은 공격력 강화", 10, 0, "+ 200 데미지 (체력 흡수 -50)")
fortification_item_302 = fortification_item("저주받은 크티티컬 강화", 10, 0, "크리티컬 + 8% (체력 흡수 -50)")
fortification_item_303 = fortification_item("저주받은 방어구 강화", 10, 0, "체력증가 + 500 (방어력 -50)")
fortification_item_401 = fortification_item("어둠의 강화", 5, 0, "+ 25 데미 (레벨 제한 55)")


# 회복 아이템 클래스
class Re_item:
    def __init__(player_1, name, recovery, cost, count, use):
        player_1.name = name
        player_1.recovery = recovery
        player_1.cost = cost
        player_1.count = count
        player_1.use = use

# 회복의 돌
re_item_01 = Re_item("최하급 회복의 돌", 10, 1, 0, "re")
re_item_02 = Re_item("하급 회복의 돌", 50, 10, 0, "re")
re_item_03 = Re_item("중급 회복의 돌", 100, 50, 0, "re")
re_item_04 = Re_item("상급 회복의 돌", 500, 500, 0, "re")
re_item_05 = Re_item("최상급 회복의 돌", 1000, 2000, 0, "re")


# 일반 아이템 클래스
class nomal_item:
    def __init__(player_1, name, count, use):
        player_1.name = name
        player_1.count = count
        player_1.use = use

# n월의 기억
month_item_02 = nomal_item("2월의 기억", 0, "nomal")
month_item_03 = nomal_item("3월의 기억", 0, "nomal")
month_item_04 = nomal_item("4월의 기억", 0, "nomal")
month_item_05 = nomal_item("5월의 기억", 0, "nomal")
month_item_06 = nomal_item("6월의 기억", 0, "nomal")
month_item_07 = nomal_item("7월의 기억", 0, "nomal")
month_item_08 = nomal_item("8월의 기억", 0, "nomal")
month_item_09 = nomal_item("9월의 기억", 0, "nomal")
month_item_10 = nomal_item("10월의 기억", 0, "nomal")
month_item_11 = nomal_item("11월의 기억", 0, "nomal")

nomal_item_00 = nomal_item("개척자의 증표", 0, "nomal")

# 일반 판매 아이템 클래스
class nomal_item_shop:
    def __init__(player_1, name, count, cost, use):
        player_1.name = name
        player_1.count = count
        player_1.cost = cost
        player_1.use = use

month_item_01 = nomal_item_shop("1월의 기억", 0, 100, "nomal")  # 100원

# 몬스터 클래스
class monster:
    def __init__(player_1, name, liv, health, damage, recovery_health, drop_exp, drop_money, drop_item, drop_item_count,
                 spawn_point):
        player_1.name = name
        player_1.liv = liv
        player_1.health = health
        player_1.damage = damage
        player_1.recovery_health = recovery_health
        player_1.drop_exp = drop_exp
        player_1.drop_money = drop_money
        player_1.drop_item = drop_item
        player_1.drop_item_count = drop_item_count
        player_1.spawn_point = spawn_point
