...
# 무기 장착
def equip(self, use, item, sel):
    while True:
        print_item_Ver2(item[sel])
        sel_1 = input("\n{0} 무기를 장착하시겟습니까? (y / n) : ".format(item[sel].name))
        if sel_1 == "y":
            if len(self.equipment[use]) == 2:
                self.inven[use].append(self.equipment[use][1])
                if use == "def":
                    self.defence -= self.equipment[use][1].defence
                del self.equipment[use][1]
                self.equipment[use].append(item[sel])
                if use == "def":
                    self.defence += self.equipment[use][1].defence
                del item[sel]
...
# 몬스터 설정
mon_0 = monster("바실리크스",2000,45,0,12000)#501 100%
mon_1 = monster("코카트리스",1000,30,200,5000)

mon_0101 = monster("불의 정령 카샤",6,1,1,1)#502  각각 60%씩
mon_0102 = monster("물의 정령 운디네",6,1,1,1)#503
mon_0103 = monster("흙의 정령 노움",6,1,1,1)#504
mon_0104 = monster("바람의 정령 실프",6,1,1,1)#505
#1레벨 몬스터 리스트
...
mon_1001 = monster("글레이 골렘",150,10,10,10)#506 , 510
mon_1002 = monster("플레시 골렘",150,10,10,10)#507 , 510
mon_1003 = monster("아이언 골렘",150,10,10,10)#508 , 510
mon_1004 = monster("스톤 골렘",150,10,10,10)#509 , 510
#10레벨 몬스터 리스트
...
mon_1501 = monster("파이어 자이언트",100,15,15,15)#511 , 515
mon_1502 = monster("프로스트 자이언트",100,15,15,15)#512 , 515
mon_1503 = monster("힐 자이언트",100,15,15,15)#513 , 515
mon_1504 = monster("스톤 자이언트",100,15,15,15)#514 , 515
#15레벨 몬스터 리스트
mon_15 = [mon_1501, mon_1502,  mon_1503, mon_1504]

mon_2001 = monster("블랙 드래곤",200,20,20,20)#516 , 521
mon_2002 = monster("블루 드래곤",200,20,20,20)#517 , 521
mon_2003 = monster("실버 드래곤",200,20,20,20)#518 , 521
mon_2004 = monster("골드 드래곤",200,20,20,20)#519 , 521
mon_2005 = monster("레드 드래곤",200,20,20,20)#520 , 521
...
#공격
def attack(pl, mon, p_m):
    if p_m == "pl":
        if len(pl.equipment['atk']) == 2:
            mon.health -= pl.equipment['atk'][1].damage
            print("{0} 데미지가 {1}에게 들어갔습니다".format(pl.equipment['atk'][1].damage, mon.name))
        if len(pl.equipment['atk']) == 1:
            mon.health -= 0
            print("효과가 없다")
    elif p_m == "mon":
        if len(pl.equipment['def']) == 2:
            pl.health -= int(mon.damage / pl.equipment['def'][1].defence)
            print("{0}의 데미지를 받았습니다".format(int(mon.damage / pl.equipment['def'][1].defence)))
        elif len((pl.equipment['def']) == 1:
            pl.health -= mon.damage
            print("{0}의 데미지를 받았습니다".format(int(mon.damage)))
...
def mon_hello(lat, self):
    if lat == 'w' or lat == 'a' or lat == 's' or lat == 'd':
        while True:
            ...
            if per == "m":
                while True:
                    per_1 = choice(list(mon_dict.keys()))
                    if per_1 <= self.liv:
                        per_2 = choice(list(mon_dict[per_1]))
                        mon = per_2
                        print("\n야생의 {0}이(가) 나타났다\n".format(mon.name))
                        turn = 1
                        r_count = 0
...
def main():
    ...
    if in_name == "":
        print()
    p_1 = player(in_name, [], [], 60, 2000, 0, 10, 150)
    self = p_1
