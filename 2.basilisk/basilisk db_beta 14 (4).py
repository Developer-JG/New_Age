...
class player:
    def __init__(self, name, equipment, inven, health, money, defense, hunger, liv):
        ...
        self.health = health
        self.money = money
        if len(self.equipment['def']) == 1:
            self.defense = self.equipment['def'][1].defense
        else:
            self.defense = 0
        self.hunger = hunger
        self.liv = liv
...
# 공격 아이템 클래스
class atk_item:
    def __init__(self, name, damage, cost, durability, liv, use, tree):
        self.name = name
        self.damage = damage
        self.cost = cost
...
item_111 = atk_item("흑빛하늘검",75,0,75,25,"atk_c")
...
def print_item(item):
    ...
    elif item.use == 'creaft':
        print("{0:^25}{1:^10}{2:^10}{3:^10}{4:^10}".format(item.name,item.damage,item.durability \
                                                          ,item.explanation,item.liv,))
# 모든 아이템 설정
...
# 몬스터 설정
mon_0 = monster("바실리크스",2000,45,0,12000)#501
mon_1 = monster("코카트리스",1000,30,200,5000)

mon_0101 = monster("불의 정령 카샤",6,1,1,1)#502
...
def game_over():
    ...
    while True:
        ...
        if end_ans == "y":
            ...
            print("\n" * 39)
            while True:
                re_start = input("게임을 다시 시작하려면 're_start' 입력 ; ")
                if re_start == "re_start":
                    print("\n" * 39)
                    main()
        if end_ans == "x":
            print("\n게임을 계속 진행합니다.")
...
def main():
    ...
    if dv == 1:
        in_name = "dv_" + in_name
        p_1 = player(in_name, [], [], 100, 2000, [], 10, 200)
        self = p_1
        self.equipment["atk"].append(item_606)
        self.equipment["def"].append(item_210)
        print("개발자 " + in_name + "님 환영합니다.")
        time.sleep(1)
    else:
        p_1 = player(in_name, [], [], 100, 10, 0, 10, 1)
        self = p_1
